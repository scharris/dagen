...
import io.sqljson.dbmd.RelMetadata;
import io.sqljson.specs.mod_stmts.ModSpec;
import io.sqljson.specs.mod_stmts.ModSql;
import io.sqljson.specs.mod_stmts.TableInputField;
import io.sqljson.util.AppUtils;
   ...
   )
   {
      ...
      return
         "insert into " + minimalRelIdentifier(tmd.getRelationId()) + "\n" +
            "  (" + fieldNames + ")\n" +
            "values(\n" +
               StringFuns.indentLines(fieldValues, indentSpaces) + "\n" +
            ")" +
            whereCond.map(cond -> "\nwhere " + cond).orElse("");
   }
   ...
   )
   {
      ...
      return
         "update " + minimalRelIdentifier(tmd.getRelationId()) + tableAlias.map(a -> " " + a).orElse("") + "\n" +
            "set\n" +
            StringFuns.indentLines(fieldAssignments, indentSpaces) +
            whereCond.map(cond -> "\nwhere " + cond).orElse("");
   }
