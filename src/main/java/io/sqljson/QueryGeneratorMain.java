...
import io.sqljson.util.AppUtils.SplitArgs;
import io.sqljson.util.Pair;
import static io.sqljson.util.AppUtils.splitOptionsAndRequiredArgs;
import static io.sqljson.util.AppUtils.throwError;
import io.sqljson.dbmd.DatabaseMetadata;
import io.sqljson.specs.queries.QueryGroupSpec;
import io.sqljson.specs.queries.ResultsRepr;
import io.sqljson.result_types.SourceCodeWriter;
import io.sqljson.result_types.source_writers.JavaWriter;
import io.sqljson.result_types.source_writers.TypescriptWriter;
   ...
   )
   {
      ...
      for ( String opt : args.optional )
      {
         if ( opt.startsWith(langOptPrefix) ) language = opt.substring(langOptPrefix.length());
            ...
            Optionals.opt(io.sqljson.util.Files.readString(Paths.get(opt.substring(generatedTypesHeaderFileOptPrefix.length()))));
         else
      ...
      throws IOException
   {
      ...
      for ( GeneratedQuery q : generatedQueries )
      {
         for ( ResultsRepr repr: q.getResultRepresentations() )
         {
            ...
            BufferedWriter bw = io.sqljson.util.Files.newFileOrStdoutWriter(outputFilePath);
