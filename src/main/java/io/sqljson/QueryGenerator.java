...
import io.sqljson.dbmd.ForeignKey;
import io.sqljson.dbmd.RelId;
import io.sqljson.sql.dialect.SqlDialect;
import io.sqljson.result_types.GeneratedType;
import io.sqljson.dbmd.ForeignKeyScope;
import io.sqljson.specs.queries.*;
import io.sqljson.sql.*;
   ...
   */
   ...
   )
   {
      ...
      tableOutputSpec.getFilter().ifPresent(filter ->
         q.addWhereClauseEntry("(" + StringFuns.substituteVarValue(filter, alias) + ")")
      );
   ...
   )
   {
      ...
      return makeAggregatedJsonResultSql(tos, Optionals.opt(childFkCond));
   }
   ...
   )
   {
      ...
      return makeJsonObjectRowsSql(tos, Optionals.opt(parentPkCond));
   }
