...
import org.dmlgen.specs.mod_stmts.ModGroupSpec;
...
public class ModStatementGeneratorMain
{
   ...
   public static void main(String[] allArgs)
   {
      ...
      if ( !Files.isRegularFile(modsSpecFilePath) )
         throwError("Mods specification file not found.");

      Optional<Path> outputDir = args.required.size() > 2 ? opt(Paths.get(args.required.get(2))) : empty();
            ...
            InputStream modsSpecIS = Files.newInputStream(modsSpecFilePath) )
      {
         ...
         outputDir.ifPresent(path ->  {
            if ( !Files.isDirectory(path) ) throwError("Mods output directory not found.");
         });
