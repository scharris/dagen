...
import static org.sqljsonquery.dbmd.CaseSensitivity.INSENSITIVE_STORED_LOWER;
import static org.sqljsonquery.dbmd.CaseSensitivity.INSENSITIVE_STORED_UPPER;
...
public class DatabaseMetadata
{
    ...
    {
        return optn(relMDsByRelId().get(relId));
    }
    ...
    {
        return getForeignKeysFromTo(opt(relId), Optional.empty());
    }
    ...
    {
        return getForeignKeysFromTo(opt(makeRelId(schema, relName)), Optional.empty());
    }

    public List<ForeignKey> getForeignKeysFromChildrenTo(RelId relId)
    {
        return getForeignKeysFromTo(Optional.empty(), opt(relId));
    }
    ...
    {
        return getForeignKeysFromTo(Optional.empty(), opt(makeRelId(schema, relName)));
    }
    ...
    {
        RelId fromRelId = makeRelId(opt(fromSchema), fromRelName);
        RelId toRelId = makeRelId(opt(toSchema), toRelName);

        return getForeignKeysFromTo(opt(fromRelId), opt(toRelId));
    }
    ...
    {
        Optional<RelId> fromRel = opt(fromRelId);
        Optional<RelId> toRel = opt(toRelId);
    ...
    public RelId makeRelId(String possiblySchemaQualifiedRelName)
    {
        ...
        {
            schema = opt(possiblySchemaQualifiedRelName.substring(0, dotix));
    ...
    public RelId identifyTable(String table, Optional<String> defaultSchema)
    {
        ...
        if ( dotIx != -1 ) // already qualified, split it
            return
               new RelId(
                  opt(normalizeName(table.substring(dotIx+1))),
                  normalizeName(table.substring(0, dotIx))
               );
        else // not qualified, qualify it if there is a default schema
            return
               defaultSchema.map(schema -> new RelId(opt(normalizeName(schema)), normalizeName(table)))
